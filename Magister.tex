...
В качестве численного метода решения уравнения (\ref{EquilibriumEquation}) выберем метод конечных элементов с использованием изопараметрических конечных элементов \cite{Zienkiewicz}, \cite{Bathe}. Для этого на области $S$ введём сетку конечно-элементной модели $S_h$. Каждый элемент $(e) \in S_h$ содержит в себе наборы узлов $\left\{\boldsymbol{x}_i^{(e)}\right\}_{i \in I^{(e)}}$ и базисных функций $\left\{N_i^{(e)}\right\}_{i \in I^{(e)}}$ таких, что $N_i^{(e)}\left(\boldsymbol{x}_j^{(e)}\right) = \delta_{ij}$ и $\sum\limits_{i \in I^{(e)}} N_i^{(e)}(\boldsymbol{x}) = 1$, $\forall \boldsymbol{x} \in S^{(e)}$, где $I^{(e)}$ --- набор индексов узлов элемента $(e)$, $\delta_{ij}$ --- дельта-функция Кронекера, $S^{(e)}$ --- область элемента $(e)$. Для каждого элемента $(e)$ введём локальную систему координат $O\xi_1^{(e)}\xi_2^{(e)}$. Отображение из локальной системы координат $O\xi_1^{(e)}\xi_2^{(e)}$ в глобальную $O\text{x}_1\text{x}_2$ будем строить следующим образом
\begin{gather*}
...
\end{multline*}
где $S_h'$ --- аппроксимированная зона нелокального влияния. Аппроксимируем интегралы конечными суммами и наконец придём к конечному равенству, где происходит ассамблирование матрицы жёсткости и правой части
\begin{multline}
	...
	w_q \dfrac{\partial N_n^{(e)}}{\partial x_i} (\boldsymbol{x}_q) J_q
	...
	i, j, k, l = \overline{1, 2},
\end{multline}
где $Q^{(e)}$ --- набор индексов квадратурных узлов элемента $(e)$; $S_h'^{(q)}$ --- аппроксимированная зона нелокального влияния на квадратурном узле $q$; $\Gamma_h$ --- конечно-элементная модель границы области $S$; $(be)$ --- одномерные элементы заданные на границе; $w_q$ --- квадратурные веса; $J_q = \left|\det{\boldsymbol{\hat{J}}(\boldsymbol{x}_q)}\right|$ --- якобиан вычисленный в квадратурной точке $\boldsymbol{x}_q$.

Вычисление системы (\ref{StiffnessMatrix}) можно упростить записав производные от функций форм в локальной системе координат
\begin{gather*}
...
Основной проблемой, которая возникает при решении данной задачи, является аппроксимация интегрального слагаемого в (\ref{DefineSigma}), так как в матрицах элементов необходимо учитывать влияние, которое приходится на соседние элементы. Действуя по правила аппроксимации вложенных интегралов мы приходим к тому, что в уравнении (\ref{StiffnessMatrix}) порядок сумм заставляет нас аппроксимировать зону нелокального влияния для каждого квадратурного узла сетки, что не очень практично, так как выполнять поиск ближайших соседей относительно квадратурных узлов может быть весьма затратной процедурой, а хранении индексов элементов, которые попадают в зону влияния, очень дорогим. Более того, такой подход попросту сложно реализовать программно, поэтому предлагается аппроксимировать зону нелокального влияния относительно центров элементов и учитывать в расчётах те элементы центры которых попали в зону влияния. Тогда формулу (\ref{StiffnessMatrix}) можно переписать в виде
\begin{multline}
	...
	i, j, k, l = \overline{1, 2},
\end{multline}
Для успешного интегрирования, зону аппроксимации лучше всего брать несколько больше, нежели реальная зона нелокального влияния, таким образом возникает шанс того, что все квадратурные узлы, которые попали под зону влияния $S'(\boldsymbol{x})$ будут учтены. Естественно, радиус поиска следует определять исходя из грубости сетки.
...
Обход сетки, который задаётся порядком суммирования в (\ref{StiffnessMatrixElementsApprox}) не позволяет эффективно использовать возможности параллельных и распределённых систем, поэтому для эффективного распараллеливания алгоритма необходимо делать обход сетки по узлам, что позволит собирать глоабльную матрицу жёсткости построчно и каждый узел кластера сможет собирать свою часть матрицы независимо от остальных.
\begin{multline}
	\label{StiffnessMatrixElementsApprox}
	\dfrac{p_1}{2} 
	...
	i, j, k, l = \overline{1, 2},
\end{multline}
$E^{(n)}$ --- набор элементов, которым принадлежит узел $n$. 
