...
На рис. \ref{fig:LongRectStress11VarFx205} и \ref{fig:LongRectStress11VarFx200} изображены напряжения $\overline{\sigma}_{11}$ вдоль оси нагружения в различных сечениях. Несмотря на различный характер нагружений, в центре области напряжения принимают одинаковую форму, которая в нелокальной постановке, в отличие от классической, не является константой вдоль оси $\overline{x}_2$. Расчётные параметры и выбранные сечения указаны на рисунках.
...
Рассматривая центральное сечение вдоль другой оси, можем убедиться в том, что в нелокальной постановке поверхность напряжений $\overline{\sigma}_{11}$ имеет более сложную форму. Вариация весоворого параметра $p_1$ влияет на отклонение напряжений на границе, в то время как вариация радиуса нелокального влияния $r$ влияет на размах кромочного эффекта. Важно отметить, что интегральная нагрузка в каждом сечении равна приложенной, что говорит нам о выполнении принципа Сен-Венана. Результаты расчётов представлена на рис. \ref{fig:LongRectStress11VarP} и \ref{fig:LongRectStress11VarR}.
...
Вариация основных параметров семейства функций нелокального влияния (\ref{phi}) показывает, что отклонение от классического решения становится тем сильнее, чем больше параметр $p$ и достигает своего максимума при константной функции влияния. Увеличение параметра $q$, напротив, уменьшает отклонение. Данный факт продемонстрирован на рис. \ref{fig:LongRectDispVarP} и \ref{fig:LongRectDispVarQ}, где показаны не сами отклонения, а относительная разница решений. Было также выбрано сечение с самым большим отклонением, чтобы показать, что учёт нелокальных эффектов может существенно повлиять на итоговые перемещения.
...
\begin{figure}[!h]
	\center{\includegraphics[width=0.7\textwidth]{pics/Tarea.eps}}
	\caption{T-образная область с граничными условиями и интересующими нас сечениями}
	\label{fig:Tarea}
\end{figure}

Проведём серию расчётов, варьируя основные параметры модели. При вариации радиуса нелокального влияния $r$ в деформациях $\overline{\varepsilon}_{22}$, помимо эффектов связанных с прикладываемой нагрузкой, которые были описаны в предыдущем разделе на примере задачи Неймана, возникают также угловые эффекты, которые характеризуются возникающими отрицательными деформациями со стороны верхней части области. Данный эффект встречался в натурных экспериментах, которые описаны в книге Андреева \cite{Andreev}. Заметим, что при увеличении радиуса нелокального влияния, также увеличивается и размах углового эффекта, а также происходит увеличение деформаций в концентраторах. Результаты представлены на рис. \ref{fig:strain22Loc}, \ref{fig:strain22NonlocP12R005}, \ref{fig:strain22NonlocP12R01} и \ref{fig:strain22NonlocP12R015}.

\begin{figure}[!hb]
	\minipage{0.49\textwidth}
	  \includegraphics[width=\textwidth]{pics/T/strain22Loc.png}
	  \caption{Распределение напряжений $\overline{\varepsilon}_{22}$ при $p_1 = 1/2$ и $r = 0$}
	  \label{fig:strain22Loc}
	  \includegraphics[width=\textwidth]{pics/T/strain22NonlocP12R01.png}
	  \caption{Распределение напряжений $\overline{\varepsilon}_{22}$ при $p_1 = 1/2$ и $r = 0.1$}
	  \label{fig:strain22NonlocP12R01}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	  \includegraphics[width=\textwidth]{pics/T/strain22NonlocP12R005.png}
	  \caption{Распределение напряжений $\overline{\varepsilon}_{22}$ при $p_1 = 1/2$ и $r = 0.05$}
	  \label{fig:strain22NonlocP12R005}
	  \includegraphics[width=\textwidth]{pics/T/strain22NonlocP12R015.png}
	  \caption{Распределение напряжений $\overline{\varepsilon}_{22}$ при $p_1 = 1/2$ и $r = 0.15$}
	  \label{fig:strain22NonlocP12R015}
	\endminipage
\end{figure}

При рассмотрении напряжений $\overline{\sigma}_{22}$ в нижней части области можем заметить, что так же как и в задаче Неймана, здесь мы наблюдаем кромочный эффект, который характеризуется снижением напряжений вблизи границ области. Также в нелокальной постановке задачи мы наблюдаем смягчение угловых концентраторов, хотя интегрально, каждое из сечений равно приложенной нагрузке. В верхней половине области различия между локальной и нелокальной постановками не столь существенны. Результаты представлены на рис. \ref{fig:stress22Loc}, \ref{fig:stress22NonlocP12R005}, \ref{fig:stress22NonlocP12R01} и \ref{fig:stress22NonlocP12R015}.

\begin{figure}[!hb]
	\minipage{0.49\textwidth}
	  \includegraphics[width=\textwidth]{pics/T/stress22Loc.png}
	  \caption{Распределение напряжений $\overline{\sigma}_{22}$ при $r = 0$ и $p_1 = 1/2$}
	  \label{fig:stress22Loc}
	  \includegraphics[width=\textwidth]{pics/T/stress22NonlocP12R01.png}
	  \caption{Распределение напряжений $\overline{\sigma}_{22}$ при $r = 0.1$ и $p_1 = 1/2$}
	  \label{fig:stress22NonlocP12R01}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	  \includegraphics[width=\textwidth]{pics/T/stress22NonlocP12R005.png}
	  \caption{Распределение напряжений $\overline{\sigma}_{22}$ при $r = 0.05$ и $p_1 = 1/2$}
	  \label{fig:stress22NonlocP12R005}
	  \includegraphics[width=\textwidth]{pics/T/stress22NonlocP12R015.png}
	  \caption{Распределение напряжений $\overline{\sigma}_{22}$ при $r = 0.15$ и $p_1 = 1/2$}
	  \label{fig:stress22NonlocP12R015}
	\endminipage
\end{figure}
...
\begin{thebibliography}{9}
	...
	\bibitem{Andreev}  Андреев А.В. Инженерные методы определения концентрации напряжений в деталях
машин. М.: Машиностроение, 1976, 72 с.
\end{thebibliography}
